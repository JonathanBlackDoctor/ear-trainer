...
interface PlaybackControlsProps {
  ...
  loading?: boolean;
  // Controlled speed (lifted to the trainer) so the choice persists across
  // questions and applies to the auto-play of the next one too.
  speed: 1 | 0.5;
  onToggleSpeed: () => void;
  // Stop in-flight playback. While loading, the primary button becomes a stop
  // button so a long stimulus can be cut short.
  onStop?: () => void;
}
...
export function PlaybackControls({
  onPlay, onPlayReference, showReference = false, loading = false,
  speed, onToggleSpeed, onStop,
...
function PlayDefault({ play, stop, speed, toggleSpeed, onPlayReference, showReference, loading }: VariantProps) {
  return (
    <div className="flex flex-wrap items-center justify-center gap-3">
      ...
      <button className="btn-primary flex items-center gap-2 px-8" onClick={loading ? stop : play}>
        <span className="text-xl">{loading ? '⏹' : '▶'}</span>
        <span>{loading ? '정지' : '듣기'}</span>
      </button>
      <button
...
function PlayG({ play, stop, speed, toggleSpeed, onPlayReference, showReference, loading }: VariantProps) {
  return (
    <div style={{ background: G.mustard, border: G.border, boxShadow: G.shadow, padding: '18px 14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button onClick={loading ? stop : play} style={{ flex: 1, height: 64, border: `3px solid ${G.ink}`, background: G.pink, color: G.ink, fontSize: 22, fontWeight: 900, letterSpacing: '-0.02em', boxShadow: '4px 4px 0 #0a0a0a' }}>
          {loading ? '⏹  정지' : '▶  듣기'}
        </button>
...
function PlayI({ play, stop, speed, toggleSpeed, onPlayReference, showReference, loading }: VariantProps) {
  ...
  return (
    <div style={{ background: I.card, border: `1px solid ${I.hairDark}`, padding: '10px 12px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
        <button onClick={loading ? stop : play} style={{ ...mono, flex: 1, padding: '12px 0', border: `1.5px solid ${I.ink}`, background: I.ink, color: I.lime, fontSize: 13, fontWeight: 800 }}>
          {loading ? '⏹ STOP / 정지' : '▶ PLAY / 듣기'}
        </button>
        {showReference && onPlayReference && (
...
function PlayJ({ play, stop, speed, toggleSpeed, onPlayReference, showReference, loading }: VariantProps) {
  return (
    <div style={{ background: J.chrome, border: `2.5px solid ${J.ink}`, boxShadow: `4px 4px 0 ${J.ink}`, padding: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
        ...
        <button onClick={loading ? stop : play} style={{ flex: 1, padding: '10px 0', border: `2px solid ${J.ink}`, background: J.pink, color: '#fff', fontSize: 13, fontWeight: 900, letterSpacing: 1 }}>{loading ? '⏹ STOP' : '▶ PLAY'}</button>
...
function PlayM({ play, stop, speed, toggleSpeed, onPlayReference, showReference, loading }: VariantProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '8px 0' }}>
      <div style={{ position: 'relative', width: 144, height: 144 }}>
        ...
        <button
          onClick={loading ? stop : play}
          aria-label={loading ? '정지' : '듣기'}
