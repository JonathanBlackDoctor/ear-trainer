...
**Source**: `source/direction-g.jsx`. The `G` token object at the top of that file is the canonical palette.
...
**Source**: `source/direction-m.jsx`. **This direction is the most performance-sensitive** (backdrop-filter, multiple blur-filtered orbs) — gate the heaviest effects behind a low-end-device check (see Performance below).
...
├── preview.html               ← open in browser to see all 4 directions live
└── source/
...
2. Set up token files (`tokens.css` or Tailwind theme extension) from that direction's table.
3. Build the **component library** (button, card, chip, level grid, choice grid, progress, sparkline, toggle, segmented control, play controls, weak-item row, mode tile) once, in that direction.
